...
namespace BackgroundProcessing.ResourceHandlers
{
    ...
    class SolarPanel : ResourceModuleHandler
    {
        ...
        {
            return s.moduleValues.GetValue("stateString") == ModuleDeployableSolarPanel.panelStates.EXTENDED.ToString();
        }
        ...
        )
        {
            ...
            if (interestingResources.Contains(p.resourceName))
            {
                ...
                ret.Add(new SolarPanel(p.resourceName, p.chargeRate, p.powerCurve, part.position, part.rotation, panel.forward, pivot.up, p.sunTracking, p.useCurve, p.temperatureEfficCurve, (float)part.temperature));
            }
        ...
        public override HashSet<ProtoPartResourceSnapshot> HandleResource(Vessel v, VesselData data, HashSet<ProtoPartResourceSnapshot> modified)
        {
            ...
            Vector3d partPos = v.GetWorldPos3D() + position;
